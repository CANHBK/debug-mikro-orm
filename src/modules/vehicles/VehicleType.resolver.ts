...
@ObjectType()
// @ts-ignore
class VehicleTypeTableResponse extends PaginatedResponse(
  VehicleTypeCountVehicle
) {}
...
export class VehicleTypeResolver {
  ...
  async getVehicleTypes(
    ...
    let list_data: VehicleTypeCountVehicle[] = await em.find(
      VehicleType,
      { ...filterBy },
      {
        limit: perPage,
    ...
    list_data = await mapAsyncAction(
      list_data,
      async (data, index) => {
  ...
  @Mutation(() => VehicleTypeResponse)
  async deleteVehicleType(@Ctx() { em }: MyContext, @Arg("inputs") id: number) {
    const vehicleType = await em.findOne(VehicleType, { id });
    if (!vehicleType) {
      return {
        errors: [
