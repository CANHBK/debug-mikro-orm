...
import { VehicleStatus } from "@root/modules/vehicles/VehicleStatus.entity";
import { VehicleType } from "@root/modules/vehicles/VehicleType.entity";
...
@Resolver(() => Vehicle)
export class VehicleFieldResolver implements ResolverInterface<Vehicle> {
  @FieldResolver()
  name(@Root() { id, year }: Vehicle) {
    return `${id} [${year}]`;
  }
}
...
export class VehicleResolver {
  ...
  async getVehicles(
    ...
    console.log(`fieldsToRelations(info)`, fieldsToRelations(info));
  ...
  async createVehicle(
    ...
    const vehicleFuel = await em.findOne(Fuel, {
