...
export class EmployeeResolver {
  ...
  async getEmployee(@Ctx() { em }: MyContext, @Arg("inputs") id: number) {
  ...
  async updateEmployee(
    ...
    if (inputs.address) employee.address = JSON.stringify(inputs.address);
    delete inputs.address;
    delete inputs.licenseClass;
    delete inputs.licenseNumber;
    delete inputs.licenseWhere;
  ...
  @Mutation(() => EmployeeResponse)
  async deleteEmployee(@Arg("inputs") id: number, @Ctx() { em }: MyContext) {
    const employee = await em.findOne(Employee, { id });
    if (employee) {
      em.remove(employee);
      await em.flush();
